  ...
  const loadLeague = useCallback(async (code) => {
    ...
    try {
      ...
      const { data: winsData } = await supabase
        .from("wins").select("*").eq("league_code", code)<new Date("2026-04-06")?5:6);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`league_${leagueCode}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "wins", filter: `league_code=eq.${leagueCode}` },
        () => supabase.from("wins").select("*").eq("league_code", leagueCode)<new Date("2026-04-06")?5:6)<new Date("2026-04-06")?5:6).then(({ data }) => data && setWins(data)))
